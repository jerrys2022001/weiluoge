...
def run_command(cwd: Path, args: list[str], check: bool = True) -> int:
...
def main() -> int:
    ...
    try:
        ...
        command = build_publish_command(worktree_path, worktree_path, args)
        code = run_command(worktree_path, command, check=False)
        if code != 0:
            print(
                f"worktree publish failed for lane={args.lane} code={code}; retrying with local repo state",
                file=sys.stderr,
            )
            fallback_command = build_publish_command(repo_root, repo_root, args)
            run_command(repo_root, fallback_command)
        return 0
    finally:
        subprocess.run(
