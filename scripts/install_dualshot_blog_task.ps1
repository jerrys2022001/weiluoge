param(
  ...
  [string]$WindowEnd = "08:29",
  [int]$PostsPerDay = 1,
  [double]$SimilarityThreshold = 0.50,
  [string]$TaskNamePrefix = "WeiLuoGe-DualShot-Camera-Blog-Morning",
...
for ($i = 0; $i -lt $publishMinutes.Count; $i++) {
  ...
  $Args = "$PythonArgs `"$ScriptPath`" --lane dualshot --repo-root `"$RepoRoot`" --slot-offset $i --similarity-threshold $SimilarityThreshold --git-commit --git-push"
